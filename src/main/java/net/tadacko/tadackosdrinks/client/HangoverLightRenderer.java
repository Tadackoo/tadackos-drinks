...
/**
 * Client-side renderer for Hangover custom visual with sun detection.
 *
 * - Raycasts to check blocks (block light).
        ...
        if (amp >= 2) {
            ...
            if (effective >= DAMAGE_TRIGGER_INTENSITY) {
                ...
                if (now - clientLastSent >= CLIENT_SEND_COOLDOWN_MS) {
                    clientLastSent = now;
                    net.tadacko.tadackosdrinks.network.ModNetwork.CHANNEL.sendToServer(new net.tadacko.tadackosdrinks.network.RequestLightDamagePacket());
                }
            }
        }
    }
